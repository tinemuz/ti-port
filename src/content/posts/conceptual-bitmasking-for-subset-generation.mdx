...
1.  _The Outer Loop:_ `for (int i = 0; i < (1 << n); i++)` is the engine of our subset generation. `1 << n` is a concise way to write $2^n$. The loop variable `i` serves as our decimal counter, which is our bitmask.

2.  _The Inner Loop & The Bitwise Check:_ Inside, the inner loop iterates through our original numbers. The key line is `if ((i & (1 << j)) != 0)`.
...
In other problems, you might need only _unique_ subsets. In that case, you could use a `Set` data structure to store the subsets you generate, as a Set automatically filters out any duplicates.
